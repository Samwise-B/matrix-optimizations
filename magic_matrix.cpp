...
bool isPairwiseDistinct( int** matrix, int N) {
    ...
    stop = omp_get_wtime();
    runtime = stop - start;
    printf("isPairwiseDistinct runtime: %f\n", runtime);
    return false;
}
...
bool isMagicSquare(int** matrix, int N)
{
    ...
    printf("average sumRow runtime: %f\n", runtime / N);
    ...
    printf("average sumCol runtime: %f\n", runtime / N);
...
int main(int argc, char *argv[])
{
    ...
    double total_start, total_stop, total_time;
    ...
    stop = omp_get_wtime();
    runtime = stop - start;
    printf("generateMagicSquare runtime: %f\n", runtime);

    start = omp_get_wtime();
    bool is_magic_square = isMagicSquare(magicSquare, M);
    stop = omp_get_wtime();
    runtime = stop - start;
    printf("isMagicSquare runtime: %f\n", runtime);
